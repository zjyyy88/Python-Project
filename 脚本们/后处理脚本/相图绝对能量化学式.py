...
entries = [
    PDEntry(Composition("Li"), -1.921765366),   # Li单质晶胞：总绝对能-105.26 eV，含16个Li原子
    PDEntry(Composition("Bi"),  -3.897912597),    # Y单质晶胞：总绝对能-328.58 eV，含2个Y原子
    PDEntry(Composition("Y"), -6.455346885),
     PDEntry(Composition("Cl"), -2.282874801),
    PDEntry(Composition("LiCl"), -7.3857361),   # Li单质晶胞：总绝对能-105.26 eV，含16个Li原子
    PDEntry(Composition("BiCl3"),  -14.32141014),    # Y单质晶胞：总绝对能-328.58 eV，含2个Y原子
    PDEntry(Composition("YCl3"), -21.17910362),    # Cl单质晶胞：总绝对能-226.15 eV，含8个Cl原子
    
    # ② 化合物相（你的研究体系，VASP计算的晶胞总绝对能量+原子数）
    PDEntry(Composition("Li9Y3Cl18"), -129.5926164),  # Li3YCl6晶胞：总绝对能-2156.82 eV，含20个原子
    PDEntry(Composition("Li9Y2Bi1Cl18"),  -121.4773016),  # LiYCl4晶胞：总绝对能-1428.36 eV，含12个原子
...
target_comp = Composition("Li9Y3Cl18")
# 从entries中提取LiYCl4的「晶胞总绝对能」和「原子数」
target_total_energy = -129.5926164
target_n_atoms = 30
# 转换为单原子绝对能量（仅这一步简单除法，Pymatgen需要）
target_e_per_atom = target_total_energy / target_n_atoms
...
if delta_e_hull < 0:
    ...
    print(f"✅ 结论：{target_comp} 是【亚稳相】（ΔE_hull>,不可合成）")
'''
# 5. 绘制凸包相图（无修改，正常运行）
