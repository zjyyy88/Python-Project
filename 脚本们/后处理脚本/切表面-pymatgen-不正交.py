...
# 对 (1,1,1) 面做近正交重整（det=2），可显著降低面内斜晶胞复杂性
ORTHOGONALIZE_111 = True
ORTHOGONALIZE_111_MATRIX = [[1, 0, 0], [-1, 2, 0], [0, 0, 1]]
	...
	for hkl in MILLER_LIST:
		...
		for term_idx, slab in enumerate(slabs, start=1):
			for sx, sy in supercells:
				...
				orth_tag = ""
				if ORTHOGONALIZE_111 and tuple(hkl) == (1, 1, 1):
					gamma_before = slab_out.lattice.gamma
					slab_out.make_supercell(ORTHOGONALIZE_111_MATRIX)
					gamma_after = slab_out.lattice.gamma
					orth_tag = "_orth111"
					print(
						f"[hkl={hkl}, term={term_idx}, SC={sx}x{sy}] "
						f"gamma: {gamma_before:.3f} -> {gamma_after:.3f}"
					)

				fname = (
					f"POSCAR_hkl{sanitize_hkl(hkl)}_term{term_idx}_"
					f"SC{sx}x{sy}{orth_tag}_{mode}.vasp"
				)
