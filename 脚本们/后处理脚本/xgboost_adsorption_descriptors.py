#!/usr/bin/env python3
"""
XGBoost-based descriptor discovery for water adsorption energy.

Core workflow:
1) Convert element labels to physically meaningful elemental descriptors.
2) Train and evaluate XGBoost with leave-one-out CV (small dataset friendly).
3) Interpret descriptor importance via SHAP (if available) or permutation importance.
"""
...
def parse_element_label(label: str) -> tuple[str, int]:
    """
    Parse labels like "Fe3+", "Ni2+", "Co" into (symbol, oxidation_state).
    """
    ...
    if number_text and sign:
        ...
        # Conservative fallback: treat trailing number as oxidation state.
...
def build_element_descriptors(symbol: str, oxidation_state: int) -> dict[str, float]:
    """
    Build elemental descriptors using ASE periodic data.
    """
...
def prepare_dataset(
    ...
    """
    Convert raw data into model features and target.
    """
    ...
    # Fill NaN (some elements have missing vdw radius in tables).
...
def build_model(random_state: int) -> xgb.XGBRegressor:
    """
    XGBoost setup tuned for small adsorption datasets.
    """
...
def run_loocv(X: pd.DataFrame, y: pd.Series, random_state: int) -> tuple[np.ndarray, dict[str, float]]:
    """
    Leave-One-Out CV for robust small-sample evaluation.
    """
    loo = LeaveOneOut()
...
def compute_importance(
    ...
    """
    Aggregate gain/permutation/(optional) SHAP importance.
    """
...
def plot_outputs(
    ...
    """
    Save parity and feature-importance plots.
    """
    # Parity plot
    ...
    # Importance plot (prefer SHAP if available)
...
def save_summary(
    ...
    """
    Save a concise text report of model quality and descriptor ranking.
    """
