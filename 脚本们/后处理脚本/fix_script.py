content = r'''import numpy as np  # 导入numpy库用于数值计算
...
bulk_structure = ase.io.read(r'C:\Users\ZHANGJY02\PycharmProjects\PythonProject\CONTCAR') # 读取体相文件
# FIX: Removed trailing backslash which caused SyntaxError
outdir = Path(r'C:\Users\ZHANGJY02\PycharmProjects\PythonProject') # 输出目录
...
# 定义切片的层数 (注意：cut 函数的 nlayers 定义可能与 surface 不同，指沿第三矢量堆叠的单胞数)
nlayers = 18 
...
for or_ in np.arange(-0.5, 0.4, 0.01):
    ...
    # 获取当前切出的 Slab 的化学式
    # mode='all' ensures we see all atoms (e.g. Li21...)
    formula = slab_structure.get_chemical_formula(mode='all')
    print(f"origo={or_:.3f}, Formula={formula}") # 打印调试信息

    # 筛选条件：找到特定化学配比的 Slab 时停止寻找
    # 这通常是为了保证切出的表面是化学计量比平衡的，或者是特定的非极性面
    # Sort elements slightly to handle variations
    if formula == 'Cl42Li21Y7' or formula == 'Li21Y7Cl42':
        print(f"Target found: {formula}")
        found = True
        break

if not found:
    print("Warning: Target formula 'Li21Y7Cl42' not found exactly. Saving the last structure.")

# 将筛选好的结构写入 VASP 文件
# f"{'_'.join(map(str, vector))}.vasp" 生成类似 "1_-1_0.vasp" 的文件名
output_filename = outdir / f"{'_'.join(map(str, vector))}.vasp"
ase.io.write(output_filename,
             slab_structure, sort=True, direct=True)
print(f"Saved to {output_filename}")
'''

with open(r'c:\Users\ZHANGJY02\PycharmProjects\PythonProject\.venv\机器学习\切表面-llz.py', 'w', encoding='utf-8') as f:
    f.write(content)
